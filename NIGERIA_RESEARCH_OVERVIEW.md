...
seven-model quantitative stack*), and an **application layer** (an opportunity assessment for
...
└── informal_economy_ai_bitcoin/           ← APPLICATION LAYER (the opportunity)
    ...
    └── debt_cycle/                         ← ANALYSIS LAYER (macro engine + models)
        ...
        ├── 7 models (graph · VAR · SFC · scenario · GCN · positioning)
...
seven-model quantitative stack**. Start with its `README.md`; read `DATA_NOTES.md` before using
...
**The seven-model stack** (`uv run python <script>.py`; managed by `pyproject.toml`):
